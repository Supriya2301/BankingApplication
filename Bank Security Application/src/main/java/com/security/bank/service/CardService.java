...
public class CardService {
    ...
    public void modifySetting(Long cardNumber, Card cardDto) {
        ...
        Double cardLimit = cardDto.getDailyLimit();
        Card fetchedCard = optionalCard.get();
        switch(fetchedCard.getCardType()){
            case DEBIT_GLOBAL -> {
            if(cardLimit != 0 && cardDto.getDailyLimit()<= 50000) fetchedCard.setDailyLimit(cardDto.getDailyLimit());
            }
            case DEBIT_CLASSIC -> {
            if(cardLimit != 0 && cardDto.getDailyLimit()<= 40000) fetchedCard.setDailyLimit(cardDto.getDailyLimit());
            }
            case CREDIT_MASTER -> {
            if(cardLimit != 0 && cardDto.getDailyLimit()<= 100000) fetchedCard.setDailyLimit(cardDto.getDailyLimit());
            }
            case CREDIT_PREMIUM -> {
            if(cardLimit != 0 && cardDto.getDailyLimit()<= 75000)  fetchedCard.setDailyLimit(cardDto.getDailyLimit());
            }
        }
        if(cardDto.getPin()!=null) fetchedCard.setPin(cardDto.getPin());
